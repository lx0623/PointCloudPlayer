...
// 	printf("ver 的 size = %lu\n",vertices.size() * sizeof(Points));
...
// // 		//for (long long i = 0; i < (long long)100000000; i++);
...
#pragma region 全局变量
// CUDA 共享的内存对象
cudaGraphicsResource* cudaGraphicsResourcePtr;
//窗口参数
const unsigned int SCR_WIDTH = 800;
const unsigned int SCR_HEIGHT = 600;
//摄像机参数
...
#pragma region 导入数据
	...
	/*for (int i = 0; i < frame_size; i++) {
		...
		struct Points temp_point;
		temp_point.x = vertices[i].x;
		temp_point.y = vertices[i].y;
		temp_point.z = vertices[i].z;
...
#pragma region OPenGL相关
	...
	int blockSize = 256;
	int numBlocks = (numElements + blockSize - 1) / blockSize;
	//processCUDA << <numBlocks, blockSize >> > (cudaData, numElements);
	//launch_cudaProcess(numBlocks, blockSize, cudaData, numElements);
	...
	/*checkCudaErrors(cudaGraphicsGLRegisterBuffer(VAO, VBO,
